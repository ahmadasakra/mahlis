      ...
      <div className="max-w-6xl mx-auto">
        ...
        {courses.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-neutral-500 text-lg">Noch keine Kurse verfügbar.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {courses.map((course) => (
              <Link
                ...
                className="block bg-neutral-900 rounded-lg p-6 hover:bg-neutral-800 transition-colors border border-neutral-800"
              >
                <h2 className="text-xl font-bold mb-2" style={{ color: '#C3E41D' }}>
                  {course.titleDe}
                </h2>
                {course.titleAr && (
                  <h3 className="text-lg mb-3 text-neutral-400">{course.titleAr}</h3>
                )}
